...
def run(gen_data_func, calc_func, name, ut_groundtruth=None, vt_groundtruth=None, ctx_init_func=None):
    ...
    calc_func(*data_args, name=name, ut_gt=ut_groundtruth, vt_gt=vt_groundtruth)
...
p = Process(target=run, args=(gen_data_taichi, launch_taichi, "Taichi-CUDA", ut, vt, init_taichi))
p.start()
p.join()
